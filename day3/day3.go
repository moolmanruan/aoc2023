...
func numberOverlaps(n Number, p Position) bool {
	if n.pos.y != p.y {
		return false
	}
	return p.x >= n.pos.x && p.x < n.pos.x+n.w
}

func gearRatio(g Position, nn []Number) (int, bool) {
	gnums := map[Number]bool{}
	for x := g.x - 1; x <= g.x+1; x++ {
		for y := g.y - 1; y <= g.y+1; y++ {
			p := Position{x, y}
			for _, n := range nn {
				if numberOverlaps(n, p) {
					gnums[n] = false
				}
			}
		}
...
func gears(input string) []Position {
	var pp []Position
